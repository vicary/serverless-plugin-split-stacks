...
module.exports = {
  ...
  depends(resource, dependency) {
    resource.DependsOn = _.union(
      toArray(resource.DependsOn),
      toArray(dependency)
    );
  },
  ...
  nestedStackResource(stackName) {
    ...
    this.nestedStackResources[stackName] = this.nestedStackResources[
      stackName
    ] || {
  ...
  stackHasRoom(stack) {
    const { outputLimit = 200, parameterLimit = 200, resourceLimit = 500 } =
      this.config || {};
    ...
    while (todo.length) {
      ...
      if (Array.isArray(currentValue)) {
        ...
        if (Ref && Ref in this.resourcesById) {
          ...
          const id =
            typeof GetAtt === "string" ? GetAtt.split(".", 1)[0] : GetAtt[0];
    ...
    if (resourceMigration) {
      if (dependencyMigration) {
        if (resourceMigration.stackName !== dependencyMigration.stackName) {
          return handlers.ResourceAndDependencyMigrated(
            resourceMigration,
            dependencyMigration
          );
        }
  ...
  getNestedStackFiles() {
    ...
    const artifactDirectory = this.serverless.service.package
      .artifactDirectoryName;

    return fs
      .readdirSync(serverlessDirPath)
      .filter((filename) => filename.match(/nested-stack(-[0-9]+)?\.json$/))
      .map((filename) => {
        return {
          key: `${artifactDirectory}/${filename}`,
          createReadStream: () =>
            fs.createReadStream(path.join(serverlessDirPath, filename)),
        };
      });
  },
  ...
  getStackSummary(stackName) {
      ...
      if (attempt < limit) {
        return new Promise((resolve) => {
          const timeout = Math.round(
            Math.random() * delay * Math.pow(2, attempt)
          );
    ...
    return listStackResources(this.provider, stackName).then(() => {
      return stackSummary;
    });
  },
